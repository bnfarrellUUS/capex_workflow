...
def _user(client, username="u", password="secret123"):
    u = User(email=f"{username}@x.com", name=username.upper(),
             password_hash=hash_password(password), roles='["REQUESTOR"]')
    db.session.add(u)
    db.session.commit()
    client.post("/api/auth/login", json={"email": f"{username}@x.com", "password": password})
    return u
...
def test_get_profile(client, app):
    ...
    assert body["email"] == "u@x.com" and "delegate_id" in body
...
def test_set_delegate(client, app):
    u = _user(client)
    other = User(email="d@x.com", name="D",
...
def test_delegate_options_excludes_self_and_non_approvers(client, app):
    ...
    appr = User(email="ap@x.com", name="Approver",
                password_hash=hash_password("secret123"), roles='["APPROVER"]')
    db.session.add(appr)
    db.session.commit()
