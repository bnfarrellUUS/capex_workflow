...
def test_login_success_then_me(client, app):
    _seed_user()
    r = client.post("/api/auth/login", json={"email": "j@x.com", "password": "secret123"})
...
def test_login_bad_credentials(client, app):
    _seed_user()
    r = client.post("/api/auth/login", json={"email": "j@x.com", "password": "wrong"})
    assert r.status_code == 401


def test_logout_clears_session(client, app):
    _seed_user()
    client.post("/api/auth/login", json={"email": "j@x.com", "password": "secret123"})
