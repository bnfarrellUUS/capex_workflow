...
def test_user_division_relationship(app):
    ...
    user = User(email="jdoe@x.com", name="J Doe", password_hash="x", division=div)
    db.session.add_all([div, user])
    db.session.commit()
    assert user.division.name == "Field Services"
    assert div.users[0].email == "jdoe@x.com"
...
def test_self_referential_delegate(app):
    a = User(email="a@x.com", name="A", password_hash="x")
    b = User(email="b@x.com", name="B", password_hash="x")
    a.delegate = b
    db.session.add_all([a, b])
    db.session.commit()
    assert a.delegate.email == "b@x.com"
    assert b.delegates_for[0].email == "a@x.com"
...
def _make_user(app):
    u = User(email="r@x.com", name="R", password_hash="x")
    db.session.add(u)
    db.session.commit()
    return u
...
def test_must_change_password_defaults_false(app):
    u = User(email="flag@x.com", name="F",
             password_hash="x")
    db.session.add(u)
    db.session.commit()
