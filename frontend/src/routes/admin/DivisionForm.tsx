...
export function DivisionForm({
  approvers, regions, division, pending, error, onSubmit,
}: {
  approvers: AdminUser[]
  regions: Region[]
  division?: Division
  ...
  const [regionId, setRegionId] = useState(division?.region_id ?? '')

  function submit(e: React.FormEvent) {
    e.preventDefault()
    onSubmit({ number, name, active, l1_approver_ids: l1Ids, region_id: regionId })
  }
